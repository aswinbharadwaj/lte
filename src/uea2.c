...
/*
 * All the declarations that are needed for the
 * encryption process are first set
 */
...
char * uea2( u8 *key, u32 count, u32 bearer, u32 dir, u8 *data, u32 length )
{
	...
	u32 *KS;
	/*Initialisation*/
	/* Load the confidentiality key for SNOW 3G initialization */
	for (i=0; i<4; i++)
	{
	...
	/* Prepare the initialization vector (IV) for SNOW 3G initialization */
	IV[3] = count;
	//printf("\n IV[3] = 0x%x",IV[3]);
	...
	{
		sprintf(var,"0x%x",data[i]);
		strcat(encrypted,var);
	}

	//printf("\n encrypted to be put = %s", encrypted);
	//printf("\n encrypted string = %s\n", data);
	free(KS);
	return encrypted;
}
